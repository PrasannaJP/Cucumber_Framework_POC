...
public class Element<ParentPanel> {
     ...
     * without type is assigned to xpath
     *
     * @param ByLocator to wait until element found.
     * @return WebElement
     */
    public WebElement getWebElement(String ByXpath) {
        setTimeout(TIMEOUT);
        WebElement webElement = getDriver().findElement(By.xpath(locator));
        setTimeout(TIMEOUT);
        return webElement;
    }
     ...
     * @return Parent instance
     */
    public ParentPanel waitForClickableAndClick() {
        boolean isClicked;
        final int timeoutSec = TIMEOUT;
        WebDriverWait wait = (WebDriverWait) new WebDriverWait(getDriver(), timeoutSec)
                .ignoring(StaleElementReferenceException.class);
        try {
